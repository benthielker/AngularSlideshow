...
AShow = (function(props) {
	 ...
	 */
	this.slide = (function(index, slide) {
	 ...
	 */
	this.currentSlide = (function(slide) {
	 ...
	 */
	this.changeSlide = (function(delta) {
	 ...
	 */
	this.fadeToSlide = (function(newSlide) {
		...
		if (newSlide && oldSlide) {
			...
			if (newElement && newElement.length) {
				...
				newElement.css('left', oldElement.css('left'));
				newElement.css('top', oldElement.css('top'));
				newElement.css('width', this.width+'px');
				newElement.css('height', this.height+'px');
	 ...
	 */
	this.panToSlide = (function(newSlide, direction) {
		...
		if (newSlide && oldSlide) {
			...
			if (newElement) {
				...
				var oldLeft = 0,
					oldTop = 0;
				if (oldElement) {
					oldLeft = new Number(oldElement.css('left').replace('px','')).valueOf();
					oldTop = new Number(oldElement.css('top').replace('px','')).valueOf();
				}
				...
				newElement.css('left', newLeft+'px');
				newElement.css('top', newTop+'px');
				newElement.css('width', this.width+'px');
				newElement.css('height', this.height+'px');
				newElement.css('z-index', 2);
				...
				var oldHolderLeft = new Number(holder.css('left').replace('px','')).valueOf(),
					oldHolderTop = new Number(holder.css('top').replace('px','')).valueOf(),
				...
				//
				this.is_transitioning = true;
				$(holder).animate({'left':newHolderLeft+'px', 'top':newHolderTop+'px'},
					this.transition_duration,
					(function() {
						// ensure correct position after animation.
						holder.css('left',newHolderLeft+'px');
						holder.css('top',newHolderTop+'px');
